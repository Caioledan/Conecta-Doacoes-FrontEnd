...
export const useCadastroItem = () => {
  ...
  const cadastrarItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      formData.append("categoria", itemFormData.categoria.trim().toUpperCase());
      formData.append("condicao", itemFormData.condicao.trim().toUpperCase());
      formData.append("localizacao", itemFormData.localizacao.trim().toUpperCase());
      formData.append("usuarioId", "1"); // Ajustável conforme necessidade
